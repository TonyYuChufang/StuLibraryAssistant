...
@interface SLLoanBookViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation SLLoanBookViewController
...
- (void)setupBarItem
{
    ...
    SLHeaderBarItemInfo *historyItem = [[SLHeaderBarItemInfo alloc] init];
    historyItem.itemImageName = @"icon_navigationBar_history";
    historyItem.barItemClickedHandler = ^{
        [weakSelf showHistoryLoanBookViewController];
    };
    [self.headerView.rightBarItems addObject:historyItem];
}
...
{
    BlockWeakSelf(weakSelf, self);
    [[SLLoanBookDataController sharedObject] queryCurrentLoanBooksWithIncrement:NO completed:^(NSMutableArray *data, NSError *error) {
        if (error == nil) {
...
- (void)showHistoryLoanBookViewController
{
    [self.navigationController setDefaultNavType];
    SLHistoryLoanBookViewController *historyVC = [[SLHistoryLoanBookViewController alloc] init];
    [self.navigationController pushViewController:historyVC animated:YES];
}
#pragma mark - UITableView Delegate & DataSource
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    return [SLLoanBookDataController sharedObject].currentLoanBooks.count;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SLLoanBook *loanBook = [[SLLoanBookDataController sharedObject].currentLoanBooks objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SLLoanBook *loanBook = [SLLoanBookDataController sharedObject].currentLoanBooks[indexPath.row];
